# 2026-08-05 — 주간 자기감사 (self-audit, 기계 생성)
...
| C. vs KOSPI (2026-05-20~) | 계좌 -3.487% vs KOSPI -14.71% → **격차 11.22%p** |
| D. 스톱 휩쏘 | 손실 스톱 7건 중 t+5 채점 7건 · 휩쏘 5건 (71.4%) · 일실 합계 +222,731원 |
| E. 게이트 위반 | 총 0건 (provenance 0 · timing 0 · chase 0 · shock 0 · card 0) |
| F. 패치 vs 검증 | policy v2.27 (직전 감사 v2.24) · 신규 왕복 0건 |
| G. 배치 | 주식 49.3% (tier bull, 목표 [65, 80]) · heat 잔여 64982원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 71.4% — 노이즈 저점 매도 반복 | 5주째 | observe — 재확인(08-03): ratchet_shadow 승격 기준 여전히 미달 — breach 확정 1/3건·관측  |
| `deployment-below-band` | 주식비중 49.3% < 목표 하한 65% — 만성 미배치 | 5주째 🔴overdue | **무처분** (패치 처분 후에도 재발 — 보완 효과 없음, 재상정) |
| `patch-without-validation` | 검증 표본 없는 정책 패치 — 패치 동결 규칙 발동 | 1주째 | **무처분** |

## ✅ 이번 주 해소 확인 (보완 검증)

- `benchmark-gap-widening` vs KOSPI 격차 악화 14.68%p → 2.43%p — 지표 정상화로 자동 해소
