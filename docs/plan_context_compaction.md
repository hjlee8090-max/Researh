...
## 4. 후속 작업
- ✅ **(2026-06-12 완료) 0900 프롬프트 감량 1차**: 사고 이력 서술 → lessons archive 참조 치환 + policy 중복 산문 압축, 26개 수술 치환(매칭 실패 시 중단 방식). 54,853B → **49,218B (-10.3%)**. 검증: 파서 고정 문자열·정책 키·스크립트 호출·임계값·trade_log 필드 등 60+ 필수 토큰 전수 보존, check_lessons_applied hard=0 soft=0 불변, audit 정상. **룰·게이트·순서·수치 무변경 — 서술만 압축.**
- (잔여) 35KB 도달은 구조 변경(룰 본문의 policy 단일 소스화 + 슬림 프롬프트) 필요 — 컴플라이언스 모델이 바뀌는 별도 의사결정. audit INFO 가 크기를 계속 추적.
