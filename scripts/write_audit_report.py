...
def humanize_audit_line(line: str) -> str:
    ...
    translations = [
        ...
        ("콘텍스트 예산 만성 초과", "루틴이 매번 읽는 파일이 2주 이상 연속으로 너무 큰 상태입니다 — 감사가 실패 처리됐습니다. 압축·이관을 실행하거나, 불가능하면 일요일 정책 리뷰에서 크기 기준 재조정을 결정해야 합니다: "),
        ("핫패스 콘텍스트 예산 초과", "루틴이 매번 읽는 파일이 너무 커졌습니다. 압축 스크립트(compact_state.py) 실행이 필요합니다: "),
        ("핫패스 콘텍스트 예산", "루틴이 매번 읽는 파일들의 크기는 모두 정상 범위입니다."),
        ("프롬프트 순증", "루틴 프롬프트가 일주일 전보다 커졌습니다 — 새 규칙을 넣을 때 낡은 문구를 함께 빼는 관례가 지켜지는지 확인이 필요합니다: "),
        ("주간 아카이브가 기계 폴백", "주간 아카이브가 자동 폴백(요약 모음)으로 대체됐습니다 — 일요일 21시 아카이브 루틴이 실행되지 않은 신호이니 등록 상태를 확인해 주세요: "),
