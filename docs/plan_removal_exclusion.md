...
- B-2. ✅ **구현 완료 (2026-08-13, policy v2.33 — 범위 확정: 인덱스형 폴백)**
  `build_week_archive_fallback.py` — 미발화 주(최근 4주·일 21:30 경과 확정분)의 아카이브를
  각 슬롯 리포트의 '한눈에 보기' 원문 인덱스로 소급 생성(멱등). weekly_compact.yml(매일)이 실행.
  **미발화 신호 보존이 핵심 설계**: 폴백 파일의 `fallback-index` 마커를 audit 이 감지해
  "기계 폴백으로 대체됨 — 루틴 미발화" WARN 을 유지한다(파일 존재가 실패를 침묵시키지 않는다).
  검증: W32 미발화분이 27개 슬롯 리포트에서 소급 생성·재실행 무동작·감사 폴백 WARN 발화 확인.
  정식 LLM 응축이 생성되면 폴백을 덮어써도 된다.
...
> ✅ **구현 완료 (2026-08-13, 사용자 승인 후 — policy v2.33)** — 연속 14일(≈10영업일) 초과 시
> FAIL 승격, streak 은 `state/context_budget_state.json` 영속(pipeline_audit 워크플로 커밋,
> 예산 복귀 시 해소·3일+ 공백 시 리셋). WARN 단계부터 "(연속 N일째)" 표기. FAIL 은 리포트·카톡
> 발송을 마친 뒤 런만 빨갛게 만든다(기존 품질/전송 분리 구조 — 알림 경로 무영향).
> 검증: 15일 시뮬레이션 FAIL 발화·5일 공백 리셋·같은 날 중복 실행 멱등 확인.
> **초기 상태를 오늘(2026-08-13)로 커밋 — 5개 초과 항목의 14일 시계가 머지 시점부터 돈다.**
> 감량 불가 항목(policy 등)은 첫 FAIL 전에 임계 재조정을 정식 패치로 결정해야 한다(§5-D 잔여).
...
> ✅ **구현 완료 (2026-08-13, policy v2.33)** — §3 순증 금지 관례 명문화 + audit 이 프롬프트
> 크기 스냅샷(35일 보존창)을 유지하며 7일+ 전 대비 +1KB 이상 순증을 INFO 로 표면화.
> 검증: 8일 전 -5KB 베이스라인 주입 시 "프롬프트 순증 ... +5,000B" INFO 발화 확인.
...
3. **3주차+ (P1~P2)**: ✅ 완료(2026-08-13) D 1차(별도 커밋)·F(사용자 승인 2026-08-13)·G·B-2.
   **P0~P2 전 항목 구현 종료.** 잔여 열린 결정 1건 — §5-D 잔여: 예산 미도달 항목
   (policy 130KB·lessons 279KB 등)은 F 의 14일 시계가 머지 시점부터 돌므로, 첫 FAIL 전에
   일요일 리뷰가 ①추가 감량(규범 purpose 분리 재작성 — 승인 필요) ②임계 재조정(근거 명기
   정식 패치) 중 하나를 결정해야 한다. 이것이 F 의 설계 목적이다(만성 WARN 방치 종식 —
   결정을 미루는 비용이 구조적으로 0 이 아니게 됨).
