...
- **09시 routine 시작은 09:20으로** 맞춘다. 09:07 개장 직후 시초가 스냅샷을 받아 반영하는 흐름이며,
  09:20 시점 스냅샷 age 는 약 13분 — `policy.data_freshness` 의 fresh(≤20분) 이내라 웹 교차확인 없이
  그대로 매매에 쓴다. (09:07 호출이 스냅샷을 커밋하는 데 ~1~2분 걸리므로 더 일찍 시작하면 08:55 전일
  종가를 읽을 위험이 있었으나, 09:20 은 그 race 가 없다 — 기존 09:00 발화가 09:07 수집 전이라 당일
  시가를 못 받던 원인을 제거.)
