# 2026-09-22 — 주간 자기감사 (self-audit, 기계 생성)
...
|---|---|
| A. 원장 정합성 | ✅ 일치 |
| B. 계좌 성과 | 왕복 25건 · 승률 36.0% · PF 0.48 · 순실현 -356,020원 |
| C. vs KOSPI (2026-05-20~) | 계좌 -7.9326% vs KOSPI -1.33% → **격차 -6.6%p** |
...
| G. 배치 | 주식 43.8% (tier strong_bull, 목표 [80, 95]) · heat 잔여 302922원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 77.8% — 노이즈 저점 매도 반복 | 12주째 | defer — 재확인(09-20) — score_ratchet_shadow.py 재실행 결과 09-13과 수치 불변(bre |
| `deployment-below-band` | 주식비중 43.8% < 목표 하한 80% — 만성 미배치 | 12주째 | observe — 재확인(09-20) — 주식비중 48.3%(09-13 37.3% 대비 회복, 여전히 목표 하한 65% 미달) |
| `lessons-balance` | lessons.md 756,732B > 예산 60,000B — 이관 처리량 부족 | 6주째 🔴overdue | **무처분** (패치 처분 후에도 재발 — 보완 효과 없음, 재상정) |

## ✅ 이번 주 해소 확인 (보완 검증)

- `benchmark-gap-widening` vs KOSPI 격차 악화 -4.64%p → -7.01%p — 지표 정상화로 자동 해소
