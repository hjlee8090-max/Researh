...
## 5. 일일 리포트 작성
`reports/YYYY-MM-DD.md` 경로로 새 파일을 작성 (초보자 친화, 한국어).
...
## 오늘 시장은 어땠을까? (초보자 설명)
- 매크로 이슈 1~2개를 **용어 풀이와 함께** 설명 (예: "FOMC가 뭔지 1줄로", "환율이 오르면 수출주에 좋은 이유")
- 외국인/기관 수급 한줄
...
- 내 의견 (애널리스트 관점): 매수 추가 / 홀드 / 비중 축소 / 매도
- **초보자 한줄 설명**: 이 종목을 왜 들고 있는지 / 왜 파는지 한 줄
...
## 오늘 배운 것 (자기보완 노트)
- 오차 발생 종목: ... (사유 분류)
- lessons.md에 추가한 교훈: (있다면 1~3줄)
- 누적 패턴 경고: (있다면)

## 내일 액션 플랜
- 09시에 점검할 종목: ...
- 신규 후보 자리: (청산 발생 시) — 어떤 섹터·테마 위주로 볼지
- 주의해야 할 매크로 이벤트: ... (FOMC, 옵션만기, CPI 발표 등 다가오는 일정)
...
git -c user.name="kospi-autoflow-bot" -c user.email="hjlee8090@gmail.com" \
    commit -m "report: YYYY-MM-DD 일일 리포트 + lessons 갱신" || true
