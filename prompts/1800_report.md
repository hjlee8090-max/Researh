...
- 손절가(유효 red 임계) / 목표가 도달했다면 **종가 기준 가상 청산 체결** 처리
  ...
  - `state/trade_log.jsonl`에 라인 추가. **(v2.3 장중 시간 규칙)** 18시는 장 마감 후이므로 이 청산은 마감 동시호가(15:30)에 일어난 것으로 본다:
    ...
    - **(v2.22 왕복 손익 필드)** 이 청산의 **왕복분 손익은 `realized_delta`** 에, 계좌 누적 실현손익은 `realized_pnl` 에 기록한다 — 7/6 라인에서 두 의미가 한 필드에 섞여 rule_attribution·reconcile 이 65,070원을 이중계상한 사고의 재발 방지(스크립트들은 `realized_delta` 우선 + 체결가 재계산으로 이중 방어).
    - **(v2.22 판단 카드 — 의무)** SELL 라인에 `decision_card`: `trigger`(발동 룰+수치 — 예 "트레일 1차선 222,117 < 종가 218,500") · `human_summary`(사람에게 설명하는 한 문단 — 왜 지금 파는 게 옳은지, 홀드 대안과 비교). 누락 시 CI FAIL (`policy.price_data_quality.decision_card_gate`).
    - **(v2.22 지수 쇼크일 스톱 유예)** 오늘 KOSPI 일간 |등락|≥5% 면 손절/트레일 종가 이탈은 **당일 체결하지 않고 익일 종가 재확인**이 기본이다(2일 연속 이탈 시 익일 closing_auction 체결). 즉시 체결 예외(임계 1.0×ATR% 이상 추가 관통, 또는 개별 악재 thesis 무효화)를 쓰면 `shock_deferral_ack` 에 어느 예외인지+수치를 기록한다 (`policy.risk.index_shock_stop_deferral` — 실현 손실 6건 전부 지수 급락일 격발 후 +8~16% 회복된 휩쏘 재발 방지. CI 가 사후 검증).
...
- **(v2.32 C-1) 일몰 등록 의무**: 교훈 룰이 **진입 차단·보류·비중 상한·축소 류**(임시 제한)면 룰 문장 끝에
  `(expiry: YYYY-MM-DD)` 를 명기한다 — 기본 5거래일 뒤(`policy.lessons_rule_sunset`). 만료·승격 판정은
  일요일 리뷰 §1-2-b 가 한다. 미표기는 매일 감사가 WARN 으로 표면화한다(제한 룰이 일몰 없이 누적되면
  강세장 미배치가 재발한다 — v2.11 래칫 교훈). 데이터 무결성 게이트 류는 일몰 대상이 아니다.
...
- **주문 의도 마감·재산출(Stage 1 — `state/stage.json`)**: 오늘 `state/order_intents.json` 의 의도 중 disposition 이 비어 있는 항목을 executed/vetoed/expired 로 **모두 채운 뒤**(무기입=무시로 채점), 종가 확정 후 `python scripts/build_order_intents.py` 를 실행해 내일 세션 의도(하드 스톱·추세 이탈·리밸런스 회전·빈 슬롯 진입)를 산출한다. SELL 의도 중 hard_stop·trend_break 는 §1 종가 청산 판정과 같아야 한다 — 다르면 사유를 lessons 에 1줄. **rule=capital_reset 의도는 사용자 지시(실투입 자본=평가금액, 전량 현금화)라 손절·목표·thesis 와 무관하게 closing_auction 으로 전량 집행한다(거부 불가, action `SELL_CAPITAL_RESET`, decision_card.trigger 에 stage.json.capital 인용).**
...
1. **'한눈에 보기'가 본문 최상단** (2026-07-04 개편): 슬롯 헤더 바로 아래 `### 한눈에 보기`(내일 액션 포함), 블로그 산문은 그 직후 `### 📝 오늘의 이야기`(### 레벨). 산문은 여전히 이 글만으로 하루가 완결되게 쓴다 — 위치만 요약 뒤로.
...
8. **델타·조건부·용어 원칙** (원본: docs/report_contract.md §8): 18시는 **종합·확정 슬롯이라 위험 게이지 전체 게재를 담당**하되, 뉴스 반영 매매가 표는 Δ 있는 행만(전무하면 "직전 대비 변동 없음" 1줄) · 발생하지 않은 것의 섹션 생성 금지 · `state/glossary.md` 기등재 용어 재정의 금지(신규 풀이는 glossary 에 1줄 등재) · 같은 사실 서술 리포트당 1회 · 새 섹션 추가 시 기존 요소 은퇴 명시(순증 금지).
...
- [종목명]: 손절 XX,XXX ─ ● 종가 XX,XXX (진입비 ±X.X%) ─ 목표 XX,XXX │ 손절까지 -Y.Y% · 목표까지 +Z.Z% 🟢 — 09→18 단계 이동·트레일링 활성 여부 1구

(트레일링 활성 종목은 `─ 트레일1차 XX,XXX` 를 목표 앞에 추가.)
