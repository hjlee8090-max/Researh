...
> **(v2.16 — ATH권+금요일 목표권 종가트리거 재확인)**: 보유 종목이 ① KOSPI 사상 최고권·VKOSPI 80선 위이고 ② 금요일이며 ③ 장중 고가가 목표가 ±2% 이내에 진입한 경우, **"오전 목표선 일시 상회는 종가 확정 전 익절 단정 금지"** 1줄을 해당 종목 코멘트에 반드시 명시한다. ATH·과열+금요일 조합에서 장중 목표 일시 상회가 오후 급반락으로 이어지는 패턴 확인(2026-06-19 삼성전자 374,500→354,000). 트레일링스톱이 익절을 관리하게 두고 종가 트리거 청산 룰(`policy.market_hours.eod_settlement`)을 우선한다.
