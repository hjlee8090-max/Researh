# 2026-09-18 — 주간 자기감사 (self-audit, 기계 생성)
...
| C. vs KOSPI (2026-05-20~) | 계좌 -8.639% vs KOSPI -5.0% → **격차 -3.64%p** |
...
| G. 배치 | 주식 48.2% (tier bull, 목표 [65, 80]) · heat 잔여 262689원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 77.8% — 노이즈 저점 매도 반복 | 11주째 | defer — 재확인(09-13) — score_ratchet_shadow.py 재실행 결과 09-06과 수치 불변(bre |
| `deployment-below-band` | 주식비중 48.2% < 목표 하한 65% — 만성 미배치 | 11주째 | observe — 재확인(09-13) — 주식비중 37.3%(09-06과 동일) < 목표 하한 65%, heat 잔여 238, |
| `lessons-balance` | lessons.md 715,775B > 예산 60,000B — 이관 처리량 부족 | 6주째 🔴overdue | **무처분** (패치 처분 후에도 재발 — 보완 효과 없음, 재상정) |

## ✅ 이번 주 해소 확인 (보완 검증)

- `benchmark-gap-widening` vs KOSPI 격차 악화 -1.12%p → -3.58%p — 지표 정상화로 자동 해소
