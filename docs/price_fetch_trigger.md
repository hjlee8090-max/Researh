...
- **1순위(주 트리거)**: 외부 스케줄러(cron-job.org 등)가 routine 5분 전에 GitHub API로
  ...
  - **단, 09시(개장 점검)만 예외**: "routine 5분 전 = 08:55"는 개장(09:00) 전이라 당일 시세가
    아직 없어 전일 종가만 담긴다(`today_ohlc=null`). 그래서 09시 슬롯은 **개장 직후 09:15** 에도
...
- **백업(안전망)**: `fetch_prices.yml`의 `schedule` cron. 외부 트리거가 멈췄을 때만 의미가 있으며,
  ...
  + **09:15**(개장 직후, 시초가) — 이고, 오후는 11:05·14:05·17:05 1겹씩이다.
...
본문:   {"ref":"main"}
시간(Asia/Seoul, 월~금): 08:55 / 09:15 / 11:55 / 14:55 / 17:55
```

- **09:15(개장 직후)는 시초가 확보용**이다. 08:55(개장 전)는 전일 종가만 담기므로 09시 리포트가
  당일 시가를 받으려면 이 호출이 필요하다(08:55는 개장 전 프라이밍용으로 유지 — 둘 다 둔다).
- **09시 routine 시작은 09:20으로** 맞춘다. 09:15 호출이 스냅샷을 커밋하는 데 ~1~2분 걸리므로,
