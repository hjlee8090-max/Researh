...
- **🌄 06:00**: 자정 예측 확정 판정(적중/정정) 1줄 + 갱신된 개장 갭 예측 1줄 (매매 없음 — 파일 없으면 "(06시 미실행)")
...
### 2-2. 빠진 슬롯·빠진 섹션
- 어느 슬롯 파일이 없으면 "(N시 미실행)" / 휴장 생략이면 "(N시 휴장 생략)" 으로 1줄 명시(복구 계약 — report_contract §7). 사후 검증·복기에 필요.
- 델타 원칙(report_contract §8)에 따라 09/12/15시에 위험 게이지·매매가 표가 없는 것은 **정상**이다 — "(섹션 없음)"으로 적지 말고 해당 정보는 18시 확정본에서 추출한다.

### 2-3. 레거시 이슈 이월 금지
- "다음주로 넘기는 watch_items" 와 "미해결 이슈" 에는 **현재도 열려 있는 이슈만** 적는다. 이미 해결된 가격 수집 이슈(2026-05-26 네이버+Yahoo 2출처 수집으로 fetch 차단·403·data confidence=low 해소)는 원본 리포트에 등장하더라도 "해결됨"으로 1줄 처리하고 미해결 이슈로 복제하지 않는다. `stale` 키는 직전 정기 수집본 보존을 뜻할 뿐 그 자체로 low 가 아니다 — **stale ≠ low.** 신뢰도 판단은 `market_snapshot`의 `confidence` 값 기준이다.
...
### 월요일 YYYY-MM-DD
- **🌙 00:00**: ...
- **🌄 06:00**: ... (2026-07-02 신설 — 그 이전 날짜는 라인 생략)
- **🌅 09:00**: ...
- **🕛 12:00**: ...
- **🔔 15:00**: ...
- **📊 18:00**: ...
