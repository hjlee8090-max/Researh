...
이 routine의 목적은 지난주 데일리 리포트와 주말 뉴스를 결합해, 다음 주 00/06/09/12/15/18 routine이 이어받을 **살아있는 주간 작전판**을 만드는 것이다.
...
## 5. 데일리 리포트 연결 설계
다음 주 00/06/09/12/15/18 routine이 같은 주간 인사이트를 이어받도록 `weekly_plan.daily_bridge`를 갱신한다.
...
> **레거시 신뢰도 서술 이월 금지**: `watch_items`·`daily_bridge` 작성 시 과거 리포트의 "fetch 차단 / stooq·Yahoo 403 / data confidence=low / 신규 진입 보류" 서술을 복제하지 않는다 (2026-05-26 네이버+Yahoo 2출처 수집으로 해결됨). 신뢰도·진입 가능 여부는 최신 `market_snapshot.json` 의 `confidence`·`entry_filter.passes` 만 근거로 한다(1순위). `stale` 키는 직전 정기 수집본 보존을 뜻할 뿐 그 자체로 low 가 아니다 — **stale ≠ low.**
