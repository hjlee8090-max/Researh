...
- **내일 시나리오(if-then) 도출**: 내일 가장 가능성 높은 갈림길 2~3개를 "조건 → 행동" 쌍으로 정해 §5 리포트 '내일 액션 플랜'의 if-then 표에 기록한다. 조건은 다음날 09시가 기계적으로 판정할 수 있게 **검증 가능한 수치·이벤트**(지수 레벨, 발표 결과, 가격 임계)로 쓴다 — "시장이 안 좋으면" 같은 모호한 조건 금지. 09시 routine 이 각 조건의 충족 여부를 판정해 그대로 실행·검증한다(개장 직후 감정 개입 차단).
