# 2026-08-23 — 주간 자기감사 (self-audit, 기계 생성)
...
| C. vs KOSPI (2026-05-20~) | 계좌 -6.175% vs KOSPI -4.11% → **격차 -2.06%p** |
| D. 스톱 휩쏘 | 손실 스톱 9건 중 t+5 채점 7건 · 휩쏘 5건 (71.4%) · 일실 합계 +222,731원 |
| E. 게이트 위반 | 총 0건 (provenance 0 · timing 0 · chase 0 · shock 0 · card 0) |
| F. 패치 vs 검증 | policy v2.34 (직전 감사 v2.34) · 신규 왕복 2건 |
| G. 배치 | 주식 48.8% (tier bull, 목표 [65, 80]) · heat 잔여 92872원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 71.4% — 노이즈 저점 매도 반복 | 7주째 | observe — 재확인(08-09): ratchet_shadow breach 확정 2/3건(07-26 1건 대비 +1)·관측 |
| `deployment-below-band` | 주식비중 48.8% < 목표 하한 65% — 만성 미배치 | 7주째 🔴overdue | **무처분** (패치 처분 후에도 재발 — 보완 효과 없음, 재상정) |
| `policy-dead-config-disclaimers` | policy dead config: disclaimers — 어느 prompt/script/workflow 도 참조하지 않음 | 2주째 🔴overdue | **무처분** |
| `policy-dead-config-rebalance_rules` | policy dead config: rebalance_rules — 어느 prompt/script/workflow 도 참조하지 않음 | 2주째 🔴overdue | **무처분** |
| `policy-dead-config-weekly_cycle` | policy dead config: weekly_cycle — 어느 prompt/script/workflow 도 참조하지 않음 | 2주째 🔴overdue | **무처분** |
| `lessons-balance` | lessons.md 390,977B > 예산 60,000B — 이관 처리량 부족 | 2주째 🔴overdue | **무처분** |
