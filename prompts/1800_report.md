...
## 5. 일일 리포트 종합·확정 (파이프라인 4단계 — 최종)
`reports/YYYY-MM-DD.md` 는 **09/12/15시 routine 이 이미 누적 작성** 한 상태다.
18시는 그 위에 **종합·확정 섹션을 추가** 하고, 파일 상단의 진행 상태 줄을 완료로 갱신한다.

### 5-1. 09/12/15시 섹션 보존
- **절대 09/12/15시 섹션을 수정하지 않는다.** (히스토리·자기보완 학습 재료)
- 만약 09/12/15 섹션 중 일부가 빠져있다면(routine 누락) → **빈 섹션을 자리표시자만 채워 보존** 하고 18시 종합에서 그 사실을 명시.

### 5-2. 18시 종합 섹션 추가
파일 끝의 `(18:00 routine에서 종합 리포트로 최종 정리)` 자리표시자를 **다음 양식으로 교체** 한다.
...
### 5-3. 진행 상태 줄 갱신
파일 최상단의 `> 진행 상태:` 줄을 다음으로 교체:
```
> 진행 상태: 09:00 ✓ / 12:00 ✓ / 15:00 ✓ / 18:00 ✓ (확정)
