...
## 4-B. Phase 2 실행 결과 (2026-07-27, policy v2.27)

| 파일 | 최초 | Phase 1 | Phase 2 | 예산 |
|---|---|---|---|---|
| `state/lessons.md` | 125.1KB | 125.1KB | **64.6KB** | 60KB (+8%) |
| `config/policy.json` | 126.8KB | 129.5KB | **116.3KB** | 95KB (+22%) |
| **09시 핫패스 총계** | **733.3KB** | 593.1KB | **519.4KB** | — |

최초 대비 **-29.2%**, 토큰 234K → 166K.

### lessons.md — 단순 절단을 버리고 구조 인식 압축으로

계획은 "섹션 1,200B 상한 + 오래된 섹션 스텁화"로 54.2KB를 예상했다. **그대로 하면 안 됐다.**

이 파일은 두 스크립트가 파싱한다. `build_lessons_index` 는 `- 분류:` 와 `다음 추론 시 고려 / 다음 적용 룰` 라인을 뽑아 주간 정책 리뷰의 1차 입력을 만들고, `check_lessons_applied` 는 `✅ codify` 마커로 교훈 반영 여부를 판정한다. 그리고 **이 라인들은 대개 섹션 끝에 있다** — 꼬리를 자르면 이것부터 날아간다.

실제로 순진한 절단을 시험했더니 **룰 3건과 분류 4건이 유실됐다.** 그래서 `scripts/compact_lessons.py` 는 기계가 읽는 라인을 전량 보존하고 서사만 걷어낸다.

검증 중 잡은 문제 둘:

1. **번호 매김 룰 불릿 유실.** `build_lessons_index` 는 "다음 ~" 트리거 라인 뒤에 이어지는 `1. 2. 3.` 불릿도 룰로 수집한다. 트리거 라인만 남기고 불릿을 버리자 2026-06-08 섹션에서 룰 3건이 통째로 사라졌다. 연속 블록 보존 규칙을 넣어 해결.
2. **`- 원인 분류:` 표기 누락.** 보존 패턴이 `- 분류:` 만 잡아 `- 원인 분류:` 로 쓴 섹션들의 분류가 '기타'로 떨어졌다. 인덱스의 정규식과 범위를 맞춰 해결.

압축 전후 파서 출력이 완전히 일치한다 — `next_rules` 57→57(유실 0), `by_category` 동일, `repeat_counter` 동일, `lessons_applied` 동일.

부수적으로 `build_lessons_index` 의 `CATEGORY_RE` 버그도 고쳤다. `원인 분류` 만 찾고 있어서 65개 섹션 중 56개가 '기타'로 분류되고 있었다 — 분류별 인덱스가 사실상 작동하지 않던 상태다. 수정 후 기타 21개.

### policy.json — `purpose` 만 이관

`purpose` 37건(18.8KB)을 `docs/policy_notes.md` 로 옮기고 본문에는 첫 문장 + `purpose_ref` 앵커만 남겼다. `changelog` 본문 보관도 5건 → 2건.

**`rule`·`*_rule` 은 이관하지 않았다.** 이 값들은 프롬프트가 내용을 직접 읽는다(`dynamic_exit_model.target_price_rule` 등). 참조로 바꾸면 프롬프트가 규칙을 못 읽는다. 그래서 계획의 20.9KB 대신 14.2KB만 회수했다.

`check_lessons_applied` 의 haystack 에 `docs/policy_notes.md` 를 추가했다. 이걸 빠뜨리면 **이미 반영된 교훈이 '앵커 미발견 = 미반영' 으로 오탐된다** — 계획에서 미리 짚은 함정이고 실제로 필요했다.

### 잔여 초과 3건과 그 이유

예산 초과가 5건 → 3건으로 줄었다. 남은 셋은 각각 구조적 한계가 있어 `alert_decisions.jsonl` 에 **만료일 있는 예외승인**(2026-08-17)으로 기록했다.

| 파일 | 초과 | 이유 |
|---|---|---|
| `policy.json` 116.3KB | +22% | `rule` 산문은 프롬프트가 읽어 이관 불가 |
| `lessons.md` 64.6KB | +8% | 구조 보존 압축의 바닥 — 파서 입력을 전량 보존한 결과 |
| `inference_checklist.md` 10.3KB | +158% | Phase 3 일몰 집행 대상 |

`lessons.md` 를 60KB 아래로 내리려면 파일 분할(`lessons_active.md` 20룰 + `theses/` + archive)이 필요하고, 이건 프롬프트 변경을 수반하는 별개 작업이다.
