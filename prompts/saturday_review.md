...
- **레거시 신뢰도 서술 이월 금지**: 분석 질문 6번("가격 신뢰도 문제")과 "가격 데이터 신뢰도 문제" 섹션을 다룰 때, 이미 해결된 이슈(2026-05-26 네이버+Yahoo 2출처 수집으로 fetch 차단·stooq/Yahoo 403·data confidence=low 해소)를 진행 중 문제처럼 복기하지 않는다. 신뢰도는 `market_snapshot.tickers.<ticker>.confidence` 값을 1순위로 따른다. `stale` 키는 직전 정기 수집본 보존을 뜻할 뿐 그 자체로 low 가 아니다 — **stale ≠ low.** 실제 스냅샷 `confidence` 가 `low` 였던 날만 신뢰도 문제로 기록한다.
...
5. 00→06→09→12→15→18 루틴 중 어느 단계에서 판단이 끊겼는가?
...
### 리포트 가독성 원칙 (작성 전 필독)
- 독자는 "주말에 한 주를 돌아보는 주식 초보 구독자"다. 데일리 리포트와 동일 원칙(2026-07-04 개편): **한눈에 보기 요약이 최상단, 블로그 산문(📝)은 그 직후**, 등락에는 **반드시 이유(📈📉, 출처 포함)**, 내부 프로세스 로그·정책 버전 번호(v2.x)·운영 지표 나열은 싣지 않는다. glossary 기등재 용어 재정의 금지(report_contract §8).
