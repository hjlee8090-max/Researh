...
- `python scripts/estimate_target_price.py` 를 실행하여 `state/target_estimate.json` 을 만든다 (뉴스·촉매·테마·섹터 반영 **목표 매도가** 추정 + 직전 리포트 대비 변동·원인 뉴스 — `report_section_md` 와 종목별 `news_target_line` 생성). 매 routine 실행이 `target_estimate_log.jsonl` 에 1행을 쌓아 '리포트마다 변경값' 델타가 산출된다.
...
### 📰 뉴스 반영 목표 매도가 (참고 추정)
(`state/target_estimate.json.report_section_md` 의 "### 📰 뉴스 반영 목표 매도가 추정" 마크다운을 **그대로 붙여넣는다** — 보유·후보 종목의 뉴스 반영 추정 목표 매도가 + **직전 리포트 대비 변동(Δ)·원인 뉴스**가 들어 있다. 이 추정가는 watchlist 의 실제 목표가·매도 트리거를 **대체하지 않는 참고 레이어**다(이중출처 혼란 방지). 위 '종목별 09시 점검' 카드의 보유 종목 목표가 줄 아래에는 해당 종목 `estimates[].news_target_line` 한 줄을 병기한다.)
