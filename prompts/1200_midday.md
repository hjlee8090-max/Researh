...
- **🛒 매수 매력도(예약주문 진입가 대비 현재가)**: `python scripts/buy_attractiveness.py` 실행 후 `state/buy_attractiveness.json.report_section_md` 마크다운을 **그대로 붙여넣는다** (활성 예약주문 BUY 의 계획가·진입상한(계획가×1.05) 대비 현재가 위치 🟢계획가 이하/🟡상한 내(추격주의)/🔴상한 초과 — 장중 카톡 알림과 동일 수치. 지표 레이어이며 실제 체결을 대체하지 않는다. 활성 예약주문 없으면 생략).
- 매크로 한 줄
