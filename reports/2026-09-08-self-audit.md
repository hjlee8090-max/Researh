# 2026-09-08 — 주간 자기감사 (self-audit, 기계 생성)
...
| C. vs KOSPI (2026-05-20~) | 계좌 -7.3398% vs KOSPI -1.79% → **격차 -5.55%p** |
...
| G. 배치 | 주식 21.2% (tier bull, 목표 [65, 80]) · heat 잔여 323489원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 77.8% — 노이즈 저점 매도 반복 | 10주째 | defer — 정책 동결(Stage 0, state/policy_freeze.json) 중 — §1-8 재심 결과 brea |
| `deployment-below-band` | 주식비중 21.2% < 목표 하한 65% — 만성 미배치 | 10주째 | observe — 재확인(09-06) — 08-30 진단(게이트 3종 정당 작동, 국면 의존) 이후 배치가 8.5%로 오히려  |
| `lessons-balance` | lessons.md 617,991B > 예산 60,000B — 이관 처리량 부족 | 4주째 🔴overdue | **무처분** (패치 처분 후에도 재발 — 보완 효과 없음, 재상정) |

## ✅ 이번 주 해소 확인 (보완 검증)

- `benchmark-gap-widening` vs KOSPI 격차 악화 -0.09%p → -5.55%p — 지표 정상화로 자동 해소
- `ledger-mismatch` 원장 불일치 — 다른 모든 지표가 오염되므로 최우선 수정 — 지표 정상화로 자동 해소
