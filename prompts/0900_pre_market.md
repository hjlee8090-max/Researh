...
### 🛒 매수 매력도 (예약주문 진입가 대비 현재가, 참고 지표)
(`python scripts/buy_attractiveness.py` 실행 후 `state/buy_attractiveness.json.report_section_md` 의 "### 🛒 매수 매력도" 마크다운을 **그대로 붙여넣는다** — 활성 예약주문 BUY 의 계획가·진입상한(계획가×1.05, 추격 방지선) 대비 현재가 위치 🟢계획가 이하(좋은 자리)/🟡상한 내(추격주의)/🔴상한 초과. **장중 카톡 알림과 동일 수치**다. 지표 레이어이며 실제 체결을 자동 대체하지 않는다 — 활성 예약주문이 없으면 이 섹션 생략.)
