# 2026-08-25 — 주간 자기감사 (self-audit, 기계 생성)
...
|---|---|
| A. 원장 정합성 | ✅ 일치 |
| B. 계좌 성과 | 왕복 17건 · 승률 29.4% · PF 0.44 · 순실현 -358,040원 |
| C. vs KOSPI (2026-05-20~) | 계좌 -7.248% vs KOSPI -7.1% → **격차 -0.15%p** |
| D. 스톱 휩쏘 | 손실 스톱 9건 중 t+5 채점 7건 · 휩쏘 5건 (71.4%) · 일실 합계 +222,731원 |
| E. 게이트 위반 | 총 0건 (provenance 0 · timing 0 · chase 0 · shock 0 · card 0) |
| F. 패치 vs 검증 | policy v2.35 (직전 감사 v2.34) · 신규 왕복 0건 |
| G. 배치 | 주식 56.6% (tier bull, 목표 [65, 80]) · heat 잔여 191358원 |
...
| id | 항목 | 경과 | 처분 상태 |
|---|---|---|---|
| `whipsaw-high` | 스톱 휩쏘율 71.4% — 노이즈 저점 매도 반복 | 8주째 | observe — 재확인(08-23, 08-09 이후 첫 리뷰 — 08-16 미발화): ratchet_shadow breach |
| `deployment-below-band` | 주식비중 56.6% < 목표 하한 65% — 만성 미배치 | 8주째 | observe — 진단 재점검(08-23) — 병목이 이전 두 라운드(히트예산 소진 8/3, universe theme 결측  |
| `patch-without-validation` | 검증 표본 없는 정책 패치 — 패치 동결 규칙 발동 | 1주째 | **무처분** |
| `lessons-balance` | lessons.md 425,025B > 예산 60,000B — 이관 처리량 부족 | 2주째 | defer — 구조적 원인 확인 — lessons_index.archive_candidates 가 이번에도 0건이다(bui |

## ✅ 이번 주 해소 확인 (보완 검증)

- `policy-dead-config-disclaimers` policy dead config: disclaimers — 어느 prompt/script/workflow 도 참조하지 않음 — 지표 정상화로 자동 해소
- `policy-dead-config-rebalance_rules` policy dead config: rebalance_rules — 어느 prompt/script/workflow 도 참조하지 않음 — 지표 정상화로 자동 해소
- `policy-dead-config-weekly_cycle` policy dead config: weekly_cycle — 어느 prompt/script/workflow 도 참조하지 않음 — 지표 정상화로 자동 해소
