...
3. **`reports/` 의 가장 최근 파일 (= 어제 또는 직전 영업일 리포트)** — 파이프라인 연결의 핵심.
   - 특히 `## 📊 18:00 종합·확정 리포트` 섹션의 다음 하위 항목을 **그대로 따라야 한다**:
     - **내일 액션 플랜**: 09시에 점검할 종목·신규 후보 자리·다가오는 매크로 이벤트
     - **09→12→15→18 의사결정 흐름 검증** 결론
     - **오늘 배운 것 (자기보완 노트)** 의 다음 진입 룰
   - 어제 리포트가 없거나 (첫 가동) 18시 섹션이 비어있으면 그 사실을 명시하고 §2-A 신규 모드로 진행.
4. `config/watchlist.json` — 현재 추천 종목 + `next_day_plan` (어제 18시가 남긴 메모)
5. `config/portfolio.json` — 보유 현황 (어제 18시에 확정된 자산·포지션)

> **파이프라인 연결 규칙**: 어제 18시가 "내일 익절 후보" 로 표시한 종목은 09시에 **우선 점검**, "내일 손절 후보" 는 09시에 **우선 검토**. 어제 결론을 무시하고 새로 판단하지 말 것. 단, 밤사이 뉴스로 가정이 바뀌었다면 그 근거를 명시한 뒤 재판단 가능.
...
### 어제 리포트로부터 이어받기
(어제 리포트가 있을 때만. 첫 가동이면 "첫 가동 — 어제 리포트 없음" 1줄로 갈음)
- 어제 18시 결론 한 줄: ...
- 어제 "내일 액션 플랜":
  - 점검 종목: ...
  - 신규 후보 자리: ... (어제 청산 발생 시)
  - 매크로 이벤트: ...
- 어제 결론을 **그대로 따른 항목** / **변경한 항목** 명시 (변경 시 사유 1줄)
