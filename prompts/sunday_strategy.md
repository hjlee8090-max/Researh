...
일요일 routine의 목적은 주말 뉴스와 다음 주 이벤트를 분석해, 월요일 00/06/09/12/15/18 routine이 이어받을 **실행 가능한 weekly_plan**을 만드는 것이다.
...
- 추세 필터를 통과하지 못한 후보는 `config/candidates.json` 에 그대로 두되, 다음 주 thesis 의 `confirming_signals` 에 "5거래일 누적이 tier 임계(strong_bull -13%/neutral -7% 등, v2.7 레짐 적응형) 위로 회복" 같은 트리거를 명시한다.
...
- **종목 탐색(주간 발굴, v2.7)**: `python scripts/screen_universe.py` 를 실행해 `state/universe_screen.json` 을 생성한다. `config/universe.json`(테마별 대형주 모집단 ~30종목)을 상대강도(KOSPI 대비 60일 초과수익)+테마로 랭킹한 결과다. `promote_suggestions`(candidates 에 없는 주도주)를 검토해 다음 주 후보로 승격할지 결정하고(thesis·`theme_exposure` 근거 URL 동반 — 환각 방지), `rotate_out_suggestions`(KOSPI 대비 60일 초과수익 ≤ -25%p 만성 후행주)를 `candidates.json` 에서 강등·교체 후보로 표시한다(반도체 주도장에 조선·금융 후행주를 반복 보유하던 알파누수 차단). 모집단에 빠진 새 메가트렌드 대형주가 있으면 `config/universe.json` 에도 추가한다. 또한 `sector_rotation`(전 섹터 몰입)·`avoid_reentry`(avoid 섹터별)를 점검 — 몰입 충족 섹터에 **출처 있는 호재**가 확인되면 `avoid_sectors` 해제·probe 재진입을 다음 주 플랜에 반영하고(`policy.sector_rotation_reentry` — 조선 전용 아님, 모든 섹터 범용), 촉매를 반영해 `themes.json` 의 theme strength 도 갱신한다.
...
- **(v2.11) 밸류에이션 앵커 주간 갱신 의무**(`policy.valuation_anchor`): `config/valuation.json` 의 보유+후보 종목 `bps`·`eps_fwd`(1차 소스: `state/consensus.json` 의 eps_consensus)·`pbr_band_5y`/`per_band_5y` 를 갱신한다 — **모든 값은 출처 URL+게재일 동반**(`web_verify_guard.source_date_verification` 동일 적용, 분기 실적 발표 후 필수 갱신). 갱신 후 `python scripts/check_valuation_guard.py` 를 실행해 `cap_target`(목표가>천장)·`overheat_entry`(현재가>밴드 상단) 종목을 `weekly_plan.watch_items` 에 표시한다. 사이클 업종은 PBR 우선(PER 함정 — 피크 실적=최저 PER). 데이터를 못 채우면 null 유지 — verdict=skip 으로 아무것도 막지 않는다(결측이 차단 룰이 되지 않게).
...
- **레거시 신뢰도 서술 이월 금지**: 다음 주 `weekly_plan.json`(특히 `watch_items`·`daily_bridge`)을 쓸 때, 과거 리포트·이전 weekly_plan 에 남은 "fetch 차단 / stooq·Yahoo 403 / data confidence=low / 신규 진입 보류" 류 서술을 복제하지 않는다 (2026-05-26 네이버+Yahoo 2출처 수집으로 해결됨). 신뢰도·진입 가능 여부는 **최신 스냅샷의 `confidence` 와 `entry_filter.passes` 만 근거**로 기술한다. `stale` 키는 직전 정기 수집본 보존을 뜻할 뿐 그 자체로 low 가 아니다 — **stale ≠ low.**
...
```json
{
  ...
  "daily_linkage": "00/06/09/12/15/18 루틴이 각각 무엇을 확인할지"
}
```
...
- 06:00 — 미국장 마감 확정으로 자정 분류를 확정·정정, 개장 갭 예측·pending_orders 트리거를 개장 전 갱신(체결 없음)
...
### 리포트 가독성 원칙 (작성 전 필독)
- 독자는 "일요일 저녁에 다음 주를 준비하는 주식 초보 구독자"다. 데일리 리포트와 동일 원칙(2026-07-04 개편): **한눈에 보기 요약이 최상단, 블로그 산문(📝)은 그 직후**, 전망에는 근거와 출처, 내부 프로세스 로그·정책 버전 번호(v2.x)·운영 지표 나열은 싣지 않는다. glossary 기등재 용어 재정의 금지(report_contract §8).
...
(다음 주를 미리 그려보는 블로그 글 — 3~4문단 산문, 표·불릿 금지.)
- 1문단: 지난주가 남긴 것(토요 사후분석의 결론)을 1~2문장으로 이어받으며 시작
- 2문단: 다음 주를 가를 가장 큰 변수 1~2개 — "무슨 일정이 → 왜 중요하고 → 우리 종목에 어떤 영향"
- 3문단: 그래서 우리의 다음 주 작전 — 무엇을 사고/지키고/피할 것인지와 그 이유
- 4문단: 월요일 아침에 가장 먼저 확인할 것으로 맺는다
...
월요일 시나리오 (if-then — 주말~월요일 아침 갈림길 2~3개. 월요일 00시/09시가 조건 충족을 판정해 그대로 실행한다. 조건은 검증 가능한 수치·이벤트로):
| 조건 (월요일 아침까지 이것이 확인되면) | 행동 |
|---|---|
| 예: 주말 지정학 이슈 악화 + 미 선물 -1% 이상 | 신규 진입 보류, 보유 손절 이격부터 확인 |
| 예: 일요일 밤 미 선물 +0.5% 이상 안정 | 계획대로 1순위 후보 진입 검토 |
