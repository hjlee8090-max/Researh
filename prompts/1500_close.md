...
- **(v2.31) 장중 지수 쇼크 재판정 의무** (`proactive_inference.intraday_shock_rejudgment`): 오늘 장중 KOSPI |등락| ≥3%(intraday index_shock 카톡 신호 또는 이번 슬롯 실측)면 오전 예측을 '유지'로 처리하지 말고 **이 슬롯 시점 실측으로 재예측을 의무 적재**한다(inference_log 새 id append — 종가 임박치 밴드 + 익일 09시 액션 후보 재판정 포함). 근거: '사이드카 발동 후 15시 적중률 급상승' 8회 반복 — 발동 사실이 최고의 예측 입력.
